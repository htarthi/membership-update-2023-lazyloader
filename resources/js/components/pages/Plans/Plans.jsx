...
import { changeIndex, resetIsDeleteSuccess, resetIsStoreTiers, resetPlanData, resetPlanErrors } from "../../../data/features/plansDetails/plansDetailsSlice";
import { deleteTiers, storeReducer } from "../../../data/features/plans/plansSlice";
import DeleteModal from "../../GlobalPartials/DeleteModal/DeleteModal";
import PlansSkeleton from "./partials/PlansSkeleton";
import { deleteTier, getRestricatedContents } from "../../../data/features/plansDetails/planAction";
...
export default function Plans() {
    ...
    return (
        <>
                                ...
                                <Page fullWidth>
                                    <div className="plans_list_wrap">
                                        <div className="simplee_membership_container">
                                            {
                                                Object.keys(groupedData)?.map((shopify_product_id, key) => {
                                                    return (
                                                        <div className="plans_card_wrap" key={key}>
                                                            <LegacyCard>
                                                                {/* plans product detail */}
                                                                <PlanProductDetail
                                                                    plan={keyBy(plans, 'shopify_product_id')[shopify_product_id]}
                                                                />

                                                                {/* tier list */}
                                                                <PlansTierList
                                                                    planGroup={groupedData[shopify_product_id]}
                                                                    shopify_product_id={shopify_product_id}
                                                                    handleAddMembersChange={handleAddMembersChange}
                                                                    handleDeletePlan={handleDeletePlan}
                                                                    lengths={groupedData[shopify_product_id]}
                                                                    plang_key={shopify_product_id}
                                                                />
            ...
            {/* delete modal */}
            <DeleteModal active={deleteModal} setDeleteModal={setDeleteModal} deleteMethod={deletePlan} />
        </>
    );
}
