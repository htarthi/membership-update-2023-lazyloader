var simplee_membership = {
  ...
  getSubscriber: function (contract_id, init_load, isCallInit) {
      ...
      $.ajax({
          ...
          type: "post",
          data: {'_method': "post", "_token": "{{ csrf_token() }}", "customer": CustomerID, "shop": shop, "contract": contract_id, "init_load": init_load},
          success: function (res) {
            ...
            if(r.isContract){
              ...
              sessionStorage.setItem("X-sm-portal", base.utf8_to_b64(JSON.stringify(data)));
  ...
  updateSubTotalPrice: function(){
    let data = JSON.parse(this.b64_to_utf8(sessionStorage.getItem('X-sm-portal')));
    ...
    shippingPrice = (typeof data.order.shipping_lines[0] != 'undefined') ? data.order.shipping_lines[0].price : 0;
    summarySubtotal = parseFloat(subTotalPrice).toFixed(2);
    current_total_price = ( ( parseFloat(summarySubtotal) + parseFloat(shippingPrice))).toFixed(2);

    $('.sub-total-price').html( this.filterPrice(subTotalPrice, data.membership.currency_code));
    $('.current-total-price').html( this.filterPrice(current_total_price, data.membership.currency_code));
    $('.shipping-price').html( this.filterPrice(shippingPrice, data.membership.currency_code));
    $('.summary-subtotal').html( this.filterPrice(summarySubtotal, data.membership.currency_code));
  },
  filterPrice: function(price, currency){
    return (currency == 'JPY') ? Number(price).toFixed(0) :  Number(price).toFixed(2);
  },
      ...
      $.ajax({
          ...
          success: function (res) {
            ...
            $.each(states, function(val, text) {
              option[i] =  $('<option value="' + text + '" data-province="'+ val +'">' + text + '</option>');
  ...
  async saveChanges(type) {
    let simpleeContractData = JSON.parse(this.b64_to_utf8(sessionStorage.getItem('X-sm-portal')));
    let base = this;

    if(type == 'edit_shipping_address'){
      simpleeContractData.membership.ship_firstName = $('#shipping_firstname').val();
      simpleeContractData.membership.ship_lastName = $('#shipping_lastname').val();
      simpleeContractData.membership.ship_company = $('#shipping_company').val();
      simpleeContractData.membership.ship_address1 = $('#shipping_address1').val();
      simpleeContractData.membership.ship_address2 = $('#shipping_address2').val();
      simpleeContractData.membership.ship_phone = $('#shipping_phone').val();
      simpleeContractData.membership.ship_city = $('#shipping_city').val();
      simpleeContractData.membership.ship_province = $('#select_states').find(":selected").text();

      simpleeContractData.membership.ship_provinceCode = $('#select_states').find(':selected').data('province');
      simpleeContractData.membership.ship_zip = $('#shipping_postalzip').val();
      simpleeContractData.membership.ship_country = $('#select_countries').find(":selected").text();
    }
    ...
    let data = {
      ...
      deleted: [],
      line_items: (type == 'portal_all') ? simpleeContractData.membership.lineItems : [],
    }
    ...
    if(type == 'cancelled'){
        ...
        if(form){
          let required_reason = form.querySelector('input[name="required_reason"]');
          let newReason = form.querySelector('input[name="cancellation_reason"]:checked');
          if(newReason){
    ...
    $.ajax({
      ...
      success: function (res) {
        ...
        if(res.isSuccess){
          sessionStorage.setItem('X-sm-portal', base.utf8_to_b64(JSON.stringify(simpleeContractData)));
          $('#simplee_order_info').css('display', 'block');
          $('#simplee_edit_order_info').css('display', 'none');
          base.closemodal();
  ...
  showModal: function(type){
    let modalTexts = JSON.parse(this.b64_to_utf8(sessionStorage.getItem('X-sm-modal')));
  ...
  setmodalText: function(){
    ...
    sessionStorage.setItem('X-sm-modal', base.utf8_to_b64(JSON.stringify(modalText)));
  }
};
...
$(document).on("click", "#cancellation_reasons_form", function () {
    ...
    let form = document.getElementById('cancellation_reasons_form');
    let additionalReasonRadio = document.getElementById('cancellation_other_reason');
    ...
    if (form) {
        ...
        form.addEventListener('change', function(event) {
            ...
            if (additionalReasonRadio && customReasonTextbox) {
                if (additionalReasonRadio.checked && additionalReasonRadio.value == 'other') {
