...
class CheckPlan
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        } elseif (@$request->id && @$request->newtab) {
            $contract = SsContract::select('user_id')->where('id', $request->id)->first();
            if ($contract) {
        ...
        if (config('shopify-app.billing_enabled') === true) {
            if (!$shop->isFreemium() && !$shop->isGrandfathered() && !$shop->plan) {
                ...
                // dd(route('app-plan', ["name" => $shop->name]));
                // return redirect()->route('app-plan', ["name" => $shop->name]);
                // dd($shop);
                return redirect('app-plan/' . $shop->name);
            }
        }
        // return redirect('app-plan/' . $shop->name);
        return $next($request);
